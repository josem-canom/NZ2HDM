...
from potpaths_nucl import h1path, h2path, a2path, spath, thetapath, h1pathken, h2pathken, a2pathken, spathken, thetapathken

lightgrey, grey, darkgrey, red = '#CCD7DB','#96ABB3','#586D75','#C42021'
vir = cm.get_cmap('viridis')
# print(colors.to_hex(vir(0.1)))
# sys.exit()


def pot(h1, h2, a2, s):
    return (28289.9 * a2**2 + 0.114897 * a2**4 - 22.6547 * a2 * h1 + 27124. * h1**2 + 0.229795 * a2**2 * h1**2 + 0.114897 * h1**4 - 60000. * h1 * h2 + 28289.9 * h2**2 + 0.229795 * a2**2 * h2**2 - 0.100697 * h1**2 * h2**2 + 0.114897 * h2**4 - 5300.02 * s**2 + 0.375 * a2**2 * s**2 - 0.0575 * a2 * h1 * s**2 + 0.125 * h1**2 * s**2 + 0.375 * h2**2 * s**2 + 0.25 * s**4)
...
r1pathken = h1pathken/sqrt(1+tbc**2) + h2pathken/sqrt(1+tbc**(-2))
r2pathken = h1pathken/sqrt(1+tbc**(-2)) - h2pathken/sqrt(1+tbc**2)
#print(h1path[0])
#print(h2path[0])
#print(r1pathken[0])
#print(h1pathken[-1])
...
barrierken = (pot(h1pathken,h2pathken,a2pathken,spathken)-Vmin)/(1e7)
...
# plot
axes[0].contour(r1mesh, smesh, Zpot, levels=15, cmap='viridis', vmin=zmin, vmax=zmax)
axes[0].plot(r1path, spath, c=red, label='Tunneling')
axes[0].scatter(r1path[0], spath[0], marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[0].scatter(r1path[-1], spath[-1], marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[0].plot(r1pathken, spathken, '--k', label='Minimization')
axes[0].scatter(r1pathken[0], spathken[0], marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)
axes[0].scatter(r1pathken[-1], spathken[-1], marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)

axes[1].plot(r1path, r2path, c=red)
axes[1].scatter(r1path[0], r2path[0], marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[1].scatter(r1path[-1], r2path[-1], marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[1].plot(r1pathken, r2pathken, '--k')
axes[1].scatter(r1pathken[0], r2pathken[0], marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)
axes[1].scatter(r1pathken[-1], r2pathken[-1], marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)

axes[2].plot(r1path, barrier, c=red)
axes[2].scatter(r1path[0], barrier[0], marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[2].scatter(r1path[-1], barrier[-1], marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[2].plot(r1pathken, barrierken, '--k')
axes[2].scatter(r1pathken[0], barrierken[0], marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)
axes[2].scatter(r1pathken[-1], barrierken[-1], marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)

axes[3].plot(r1path, thetapath*100, c=red)
axes[3].scatter(r1path[0], thetapath[0]*100., marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[3].scatter(r1path[-1], thetapath[-1]*100., marker='*', s=50, c=colors.to_hex(vir(0.1)),zorder=5)
axes[3].plot(r1pathken, thetapathken*100, '--k')
axes[3].scatter(r1pathken[0], thetapathken[0]*100., marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)
axes[3].scatter(r1pathken[-1], thetapathken[-1]*100., marker='*', s=50, c=colors.to_hex(vir(0.5)),zorder=5)
...
axes[0].set_title(r'$\log\!\left((V-V_{\mathrm{min}})\,\mathrm{[GeV]^4}\right);\,T_N=68.76\,\mathrm{GeV}$', pad=10., fontsize=16)
...
axes[0].legend(fontsize=12, fancybox=False, framealpha=1., edgecolor='black')
...
# plt.show()
plt.savefig('potential_plot_nucl.pdf')
