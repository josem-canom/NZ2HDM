...
def add_arrow_to_leg(ax, label):
    ...
    return ax.legend(handles + [arrow], labels + [label],
              ...
              loc='upper right', fancybox=False, framealpha=.7, edgecolor='black')
...
def plane_phi_T(phases, transitions, pdf_name="plane.pdf"):
    ...
    T_max = 200
    phi_min = np.full((n_field), np.inf)
    phi_max = np.full((n_field), -np.inf)
...
#            ax.plot(phi, T, "-o", markersize=1, label="Phase ${0}$ ".format(j), **LINE)
            if j==0:
            	ax.plot(phi, T, label="Origin".format(j), **LINE)
            elif j==1:
            	ax.plot(phi, T, label="Singlet".format(j), **LINE)
            elif j==2:
            	ax.plot(phi, T, label="EW".format(j), **LINE)

            # Update minimum and maximum field values
            phi_min[i] = min(phi_min[i], min(phi))
            phi_max[i] = max(phi_max[i], max(phi))

            # Update maximum temperature
            if is_origin(phi[np.argmax(T)]):
                if is_origin(phi[np.argmin(T)]):
                    T_max = max(T_max, min(T) + 50)
                    continue
                for T_, phi_ in zip(p.T, p.phi):
                    if is_origin(phi_):
                        T_max = max(T_max, T_ + 50)
                        break
        ...
        ax.set_ylim(0, T_max)
        phi_shift = max((phi_max[i] - phi_min[i])*0.1, -np.inf)
        ax.set_xlim(phi_min[i]-phi_shift, phi_max[i]+phi_shift)

        if len(axs) == 1:
            ax.set_xlabel(r"Field $h$ (GeV)")
        else:
            if i<2:
            	ax.set_xlabel(r"Field $h_{0}$ (GeV)".format(i+1))
            elif i==2:
            	ax.set_xlabel(r"Field $a_2$ (GeV)")
            else:
            	ax.set_xlabel(r"Field $s$ (GeV)")
        if i == 0:
            ax.set_ylabel("Temperature, $T$ (GeV)")

    if transitions:
        add_arrow_to_leg(axs[-1], "  $T_C$")
